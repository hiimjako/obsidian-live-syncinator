...
export class Syncinator {
    ...
    async fetchRemoteFiles() {
        ...
        for (const file of files) {
            ...
            if (!exists) {
                ...
                const localContent = await this.storage.readText(
                    file.workspacePath,
                );
                ...
                const localFileMtime = new Date(
                    stat?.mtime ?? stat?.ctime ?? 0,
                );
                ...
                if (
                    this.options.conflictResolution === "remote" ||
                    shouldRemote
                ) {
                    log.debug(
                    ...
                    this.options.conflictResolution === "local" ||
                    shouldLocal
                ) {
                    ...
                    log.warn(
                        `conflict on file ${file.workspacePath} not solved`,
                    );
                }
            }
        }
    ...
    async onChunkMessage(data: ChunkMessage) {
        ...
        try {
            ...
            if (
                !deque.isEmpty() &&
                isSameChunkMessage(data, deque.peekFront())
            ) {
                // it is an ack
                log.debug(
            ...
            if (!deque.isEmpty()) {
                ...
                while (!deque.isEmpty()) {
                    ...
                    for (let i = 0; i < cm.chunks.length; i++) {
                        ...
                        await this.storage.persistChunk(
                            file.workspacePath,
                            inverse,
                        );
                    }
            ...
            log.error(
                `Error processing chunk message for file '${fileId}':`,
                error,
            );
        } finally {
        ...
        if (event.type === MessageType.Create) {
            ...
            if (event.objectType === "file") {
                ...
                if (!file) {
                    ...
                    const fileApi = await this.apiClient.fetchFile(
                        event.fileId,
                    );
                    this.fileCache.create(fileApi);
                    await this.storage.write(
                        fileApi.workspacePath,
                        fileApi.content,
                    );
    ...
    private async rename(file: TAbstractFile, oldPath: string) {
        ...
        if (fileToRename === undefined) {
            log.error(
                `missing file for rename: "${oldPath}", probably a folder`,
            );
